              ...
              <div className="carousel-indicators">
                {
                    [...Array(product.assets.length)].map(i =>
                      <button type="button" data-bs-target="#carousel" data-bs-slide-to={i} className="active" aria-current="true" aria-label={`Slide ${i}`} />
                    )
                }
              </div>
              <div className="carousel-inner">
                {product.assets.map(asset =>
                  <div className={`carousel-item ${product.assets.indexOf(asset) === 0 ? 'active' : ''}`}>
                    <img src={asset.url} style={{ width: "100%", objectFit: 'cover' }} />
                  </div>
                )}
              </div>
